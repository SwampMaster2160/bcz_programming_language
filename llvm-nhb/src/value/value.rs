...
use crate::{basic_block::BasicBlock, builder::Builder, context::Context, module::Module, traits::WrappedReference, types::types::Type};
use crate::enums::{CallingConvention, Linkage};
use crate::llvm_c::{LLVMAppendBasicBlockInContext, LLVMBuildCall2, LLVMBuildLoad2, LLVMSetLinkage};
use crate::llvm_c::{LLVMBuildPtrToInt, LLVMBuildRet, LLVMBuildStore, LLVMSetInitializer};
use crate::llvm_c::{LLVMCountParams, LLVMGetParam, LLVMGetValueKind, LLVMTypeOf, LLVMSetFunctionCallConv};
use crate::llvm_c::{LLVMTypeKind, LLVMLinkage, LLVMValueKind, LLVMValueRef};
...
impl<'c, 'm> Value<'c, 'm> where Value<'c, 'm>: Sized {
	...
	pub fn build_ptr_to_int(&self, builder: &Builder<'c, 'm>, dest_type: IntType<'c>, name: &str) -> IntValue<'c, 'm> {
		...
		if dest_type.as_type().pointer_to() != input_type {
			panic!("Type mismatch");
		}
		let name = CString::new(name).unwrap();
		unsafe { IntValue::from_ref(LLVMBuildPtrToInt(builder.get_ref(), self.value_ref, dest_type.get_ref(), name.as_ptr())) }
	}
...
impl<'c, 'm> TryInto<IntValue<'c, 'm>> for Value<'c, 'm> {
	type Error = ();

	fn try_into(self) -> Result<IntValue<'c, 'm>, Self::Error> {
		match self.get_type().type_kind() {
